...
from models.VideoGPT2 import *
import pickle as pkl

from dataset_Cmask import get_dataset, AVSDDataSet, collate_fn
from dataset_test import get_dataset_test, AVSDDataSet_test, collate_fn_test
...
def get_data_loaders_new(args, tokenizer):
    ...
    valid_dataset = AVSDDataSet_test(valid_data[0], tokenizer, (valid_data[1], train_data[1]), drop_rate=0, train=False)
    train_loader = DataLoader(train_dataset, batch_size=args.train_batch_size, num_workers=4, shuffle=(not args.distributed), collate_fn=lambda x: collate_fn(x, tokenizer.pad_token_id, tokenizer.mask_token_id, features=True))
    valid_loader = DataLoader(valid_dataset, batch_size=args.valid_batch_size, num_workers=4, shuffle=False, collate_fn=lambda x: collate_fn_test(x, tokenizer.pad_token_id, tokenizer.mask_token_id, features=True))
...
def train():
    ...
    parser.add_argument("--train_path", type=str, default="data/train.json", help="Path of the trainset")
    parser.add_argument("--fea_path", type=str, default="data/", help="Path of the trainset")
    parser.add_argument("--valid_path", type=str, default="data/valid.json", help="Path of the validset")
    ...
    parser.add_argument("--env_devices", type=str, default='0', help="CUDA_VISIBLE_DEVICES")
    ...
    def update(engine, batch):
        ...
        video_len = video_embs.size(1)
        text_len = input_embs.size(1)
        input_embs = torch.cat([video_embs, input_embs], dim=1)
        b = tokenizer.convert_tokens_to_ids(SPECIAL_TOKENS[-2])
        token_type_ids = torch.cat([torch.ones((i3d.size(0), i3d.size(1))).long().to(args.device) * tokenizer.convert_tokens_to_ids(SPECIAL_TOKENS[-3]), token_type_ids], dim=1)
        # token_type_ids = torch.cat([torch.ones((i3d.size(0), i3d.size(1))).long() * tokenizer.convert_tokens_to_ids(SPECIAL_TOKENS[-2]), token_type_ids], dim=1)
        ...
        reply_loss = model(input_embs,token_type_ids=token_type_ids, labels=(labels, i3d), attention_mask=[reply_mask, input_mask], mode="reply",train=1,text_len=text_len,video_len=video_len)[0]
    ...
    def inference(engine, batch):
        ...
        with torch.no_grad():
            ...
            input_embs = torch.cat([video_embs, input_embs], dim=1)
            token_type_ids = torch.cat([torch.ones((i3d.size(0), i3d.size(1))).long().to(args.device) * tokenizer.convert_tokens_to_ids(SPECIAL_TOKENS[-3]), token_type_ids], dim=1)
            model_outputs = model(input_embs, token_type_ids=token_type_ids, attention_mask=[reply_mask, input_mask],train=0,text_len=text_len,video_len=video_len)[0]
    ...
    if args.local_rank in [-1, 0]:
        ...
        checkpoint_handler = ModelCheckpoint(args.log_path, 'checkpoint', save_interval=1, n_saved=11,require_empty=False)
