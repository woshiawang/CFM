...
class Attention(nn.Module):
    ...
    def _attn(self, q, k, v, attention_mask=None, head_mask=None):
        ...
        if attention_mask is not None:
            ...
            ex2 = 1 - b
            ex3 = - 1e18 * (1 - b)
            w = w * b - 1e18 * (1 - b)# 此时w上三角负无穷
            ex4 = - 1e18 * (1 - attention_mask[1])
...
class VideoGPT2Model(GPT2Model):
    ...
    def __init__(self, config):
        ...
        self.self_attn_layer_norm = LayerNorm(nx, eps=config.layer_norm_epsilon)
        self.final_layer_norm = LayerNorm(nx, eps=config.layer_norm_epsilon)
        self.training = True
        layer_scale_init_value = 1e-2
        self.dropout_prob = 0.1
        self.gamma_1 = nn.Parameter(layer_scale_init_value * torch.ones((nx)), requires_grad=True)
        self.gamma_2 = nn.Parameter(layer_scale_init_value * torch.ones((nx)), requires_grad=True)
        self.video_ffn = self.build_geglu_ffn()
        self.text_ffn = self.build_geglu_ffn()
        # self.activation_dropout_module = FairseqDropout(
        #     float(0), module_name=self.__class__.__name__
        # )
    def build_geglu_ffn(self):
        return nn.Sequential(
            *[
                GeGLU(self.nx, self.nx * 4),
                nn.Dropout(0.1),
                LayerNorm(self.nx * 4),
                Linear(self.nx * 4, self.nx)
            ]
        )

    def forward(self, input_embs, past=None, attention_mask=None, token_type_ids=None,
                position_ids=None, head_mask=None,train=0,text_len=None,video_len=None):
        if past is None:
            past_length = 0
            past = [None] * len(self.h) # past = [None]*12
        else:
        ...
        # hidden_states = fused_dropout_res(
          #  hidden_states, self.gamma_1, residual,
           # self.dropout_prob if train else 0.0,
            #0.0
        #)
        residual = hidden_states
        hidden_states = self.final_layer_norm(hidden_states)
        video_x = self.text_ffn(hidden_states[:, :video_len, :])
        text_x = self.text_ffn(hidden_states[:, -text_len:, :])
        x = torch.cat([video_x, text_x], dim=1)
        hidden_states = fused_dropout_res(
...
class VideoGPT2LMHeadModel(GPT2PreTrainedModel):
    def __init__(self, config):
        ...
        # self.text_ff = Text_feature(config)
        self.video_ff = nn.Linear(4224, config.n_embd)
        # self.video_ff = nn.Linear(1260, config.n_embd)
        # self.video_i3d = I3D_feature(config)
        # self.video_vgg = VGG_feature(config)
        # self.video_ff1 = nn.Linear(4224, 1020)
        # self.video_ff2 = nn.Linear(1020, 768)
        # self.video_ff = Video_feature(config)
        self.init_weights()
        self.tie_weights()
    ...
    def forward(self, input_embs, past=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None,
                labels=None, mode="reply",train=0,text_len=None,video_len=None):
